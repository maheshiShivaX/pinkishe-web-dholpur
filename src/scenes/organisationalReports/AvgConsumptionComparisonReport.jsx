...
import { fetchAvgConsumptionReport, saveReport, viewSavedReport, updateSavedReport, clearSelectedSavedReport } from "../../store/reportsSlice"; // ✅ corrected import
...
const initialValues = {
    ...
    includeColumns: {
        state: true,
        schoolDistrict: true,
        averageConsumptionRange: true,
        noOfSchools: true,
        percentOfTotalSchools: true,
    },
};
...
const AvgConsumptionComparisonReport = () => {
  ...
  ========================= */
        ...
        try {
            if (reportId) {
                ...
                await dispatch(
                    saveReport({
                        ...payload,
                        reportType: "avg_consumption_comparison",
                        summary: null,
                    })
                ).unwrap();
                // ✅ SUCCESS MESSAGE (UPDATE)
    ...
    const allColumns = [
        { field: "srNo", headerName: "Sr No", width: 80, sortable: false, filterable: false, renderCell: (params) => params.api.getRowIndexRelativeToVisibleRows(params.id) + 1 },
        { field: "state", headerName: "State", flex: 1 },
        { field: "schoolDistrict", headerName: "District", flex: 1 },
        { field: "averageConsumptionRange", headerName: "Average Consumption Range", flex: 1 },
        { field: "noOfSchools", headerName: "No. of Schools", flex: 1 },
        { field: "percentOfTotalSchools", headerName: "% of Total Schools", flex: 1 },
    ];
    ...
    const handleDownloadPDF = (rows, columns, filters) => {
        ...
        doc.setFontSize(14);
        doc.text(
            "Average Consumption Comparison Report",
            pageWidth / 2,
            12,
            { align: "center" }
        );
        ...
        <Box m="20px">
            <Box display="flex" alignItems="center" gap={1} mb={2}>
                ...
                {reportId ? (
                    ...
                    <Header
                        title={"Average Consumption Comparison Report"}
                        subtitle={"Compare average pad consumption across schools"}
                    />
                )}
            </Box>
            ...
            >
                {({ values, setFieldValue }) => {
                    ...
                    }, [values, token, dispatch, viewSavedReportLoading]);
                        ...
                        <>
                            ...
                            <Box display="grid" gridTemplateColumns="repeat(4, 1fr)" gap="20px" mb="20px">
                                ...
                                <TextField
                                    ...
                                    SelectProps={{
                                        multiple: true,
                                        displayEmpty: true,
                                        renderValue: (selected) => (selected.length === 0 ? "All Districts" : selected.join(", ")),
                                    }}
                                ...
                                <TextField select label="Type of Dispense" value={values.dispenseType} onChange={(e) => setFieldValue("dispenseType", e.target.value)} sx={inputSx}>
                                    <MenuItem value="all">All</MenuItem>
                                    <MenuItem value="dispensed">Dispense through Machine</MenuItem>
                                    <MenuItem value="manual">Manual pad distribution</MenuItem>
                                </TextField>
                                ...
                                <TextField
                                    ...
                                    onChange={(e) => setFieldValue("minRange", parseFloat(e.target.value))}
                                    sx={inputSx}
                                />
                                <TextField
                                    type="number"
                                    ...
                                    onChange={(e) => setFieldValue("maxRange", parseFloat(e.target.value))}
                                    sx={inputSx}
                                />
                                {/* <TextField
                                    type="number"
                                    label="Step"
                                    value={values.step}
                                    onChange={(e) => setFieldValue("step", parseFloat(e.target.value))}
                                    sx={inputSx}
                                /> */}
                            </Box>
                            ...
                            <Box height="70vh">
                                {avgConsumptionLoading ? (
                                    ...
                                    <DataGrid
                                        rows={avgConsumptionReport?.map((row, i) => ({ id: i, ...row })) || []}
                                        columns={displayedColumns}
                                        // slots={{ toolbar: GridToolbar }}
                                        slots={{
                                            toolbar: () => (
                                                <CustomToolbar
                                                    rows={avgConsumptionReport}
                                                    columns={displayedColumns}
...
export default AvgConsumptionComparisonReport;
